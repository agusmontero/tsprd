...
namespace separation
{
	...
	pair<double, STCut> maxflow_mincut(const vector<vector<int>> &D, const function<double(int i, int j)> &c, int s, int t)
	{
		...
		for (auto e : reverse_reverses)
			reverse_arcs.push_back(e);
		...
		for (int i = 0; i < n; ++i)
		{
			if (color[i] == boost::black_color)
				min_cut.S.push_back(i);
			else
				min_cut.T.push_back(i);
		}
